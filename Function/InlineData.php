 ...
 * @subpackage	Sass.extensions.compass.functions
 */
class Compass_Function_InlineData {
	public function inline_image($path, $mime_type = null) {
		$path = $path->value;
		$real_path = Compass_Config::config('images_path').DIRECTORY_SEPARATOR.$path;
	...
	public function inline_font_files() {
		...
		while ($args) {
			...
			$real_path = Compass_Config::config('fonts_path').DIRECTORY_SEPARATOR.$path->value;
