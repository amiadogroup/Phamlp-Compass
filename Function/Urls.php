...
class Compass_Function_Urls {
	public function stylesheet_url($path, $only_path = null) {
		...
		if (Compass_Config::config('relative_assets'))
			$http_css_path = self::compute_relative_path(Compass_Config::config('css_path'));
		elseif (Compass_Config::config('http_css_path'))
			$http_css_path = Compass_Config::config('http_css_path');
		else
			$http_css_path = Compass_Config::config('css_dir');
	...
	public function font_url($path, $only_path = null) {
		...
		if (Compass_Config::config('relative_assets'))
			$http_fonts_path = self::compute_relative_path(Compass_Config::config('fonts_path'));
		else
			$http_fonts_path = Compass_Config::config('http_fonts_path');
	...
	public function image_url($path, $only_path = null) {
		...
		if (preg_match('%^'.preg_quote(Compass_Config::config('http_images_path'), '%').'/(.*)%',$path, $matches))
		...
		# or nil if the http_images_path is not set in the configuration.
		if (Compass_Config::config('relative_assets'))
			$http_images_path = self::compute_relative_path(Compass_Config::config('images_path'));
		elseif (Compass_Config::config('http_images_path'))
			$http_images_path = Compass_Config::config('http_images_path');
		else
			$http_images_path = Compass_Config::config('images_dir');

		# Compute the real $path to the image on the file stystem if the images_dir is set.
		if (Compass_Config::config('images_dir'))
			$real_path = Compass_Config::config('project_path').
				DIRECTORY_SEPARATOR.Compass_Config::config('images_dir').
